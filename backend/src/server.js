...
// ESM-Server für Chocadies Recruiting API (robust gegen fehlende openapi.yaml)
...
// kleine Header wie in deinen Outputs
...
async function initDb() {
  ...
  await Applications.createIndex(
    { deleteAt: 1 },
    { background: true, partialFilterExpression: { deleteAt: { $exists: true } }, expireAfterSeconds: 0 }
  );
  // Wichtig: unique auf (job,user)
...
async function findUserByEmail(email) {
  ...
  return await Users.findOne({ email: String(email).toLowerCase() });
}
...
function randKey(bytes = 24) {
  // robust & portabel (keine WebCrypto-Abhängigkeit)
...
app.get("/api/public/jobs", async (_req, res) => {
  const docs = await Jobs.find({ open: true }).sort({ _id: -1 }).toArray();
  res.json(docs.map(j => ({ id: j._id.toString(), title: j.title, description: j.description })));
});

app.post("/api/v2/jobs", authRequired, requireRole("admin"), async (req, res) => {
...
app.get("/api/v2/jobs", authRequired, requireRole("admin"), async (_req, res) => {
  const docs = await Jobs.find({}).sort({ _id: -1 }).toArray();
  res.json(docs);
});

app.patch("/api/v2/jobs/:id", authRequired, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  const patch = {};
  ["title", "description", "open"].forEach(k => {
    if (req.body?.[k] !== undefined) patch[k] = req.body[k];
  });
  patch.updatedAt = new Date();
  await Jobs.updateOne({ _id: new ObjectId(id) }, { $set: patch });
  res.json({ ok: true });
});

app.delete("/api/v2/jobs/:id", authRequired, requireRole("admin"), async (req, res) => {
...
app.post("/api/v2/applications", authRequired, async (req, res) => {
  try {
    ...
    console.error("applications:create", e);
    return res.status(500).json({ error: "server_error" });
  }
});
...
app.get("/api/v2/applications/me", authRequired, async (req, res) => {
  ...
  const jobs = await Jobs.find({ _id: { $in: jobIds } }).toArray();
  const jobMap = new Map(jobs.map(j => [j._id.toString(), j]));
  const out = docs.map(a => ({
    id: a._id.toString(),
    status: a.status,
    job: (() => {
      const j = jobMap.get(a.job.toString());
      return j ? { id: j._id.toString(), title: j.title, description: j.description } : null;
    })(),
...
app.get("/api/v2/applications", authRequired, requireRole("admin"), async (req, res) => {
  const { jobId } = req.query;
  if (!jobId) return res.json([]);
  const docs = await Applications.find({ job: new ObjectId(String(jobId)) }).sort({ _id: -1 }).toArray();
  ...
  const out = docs.map(a => ({
    ...
    status: a.status,
    applicant: (() => {
      const u = uMap.get(a.user.toString());
      return u ? { email: u.email, birthdate: u.birthdate || null, address: u.address || null } : null;
    })(),
    createdAt: a.createdAt,
  }));
  res.json(out);
...
app.patch("/api/v2/applications/:id", authRequired, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  const { status } = req.body || {};
  if (!status) return res.status(400).json({ error: "missing_status" });
  const r = await Applications.updateOne({ _id: new ObjectId(id) }, { $set: { status, updatedAt: new Date() } });
  if (!r.matchedCount) return res.status(404).json({ error: "Not found" });
  res.json({ ok: true, id, status });
});

app.delete("/api/v2/applications/:id", authRequired, requireRole("admin"), async (req, res) => {
...
app.get("/api/v2/meetings/me", authRequired, async (req, res) => {
  ...
  const out = docs.map(m => ({
    ...
    location: m.location,
    job: (() => {
      const j = jMap.get(m.job.toString());
      return j ? { id: j._id.toString(), title: j.title } : null;
    })(),
  }));
  res.json(out);
});

app.get("/api/v2/meetings", authRequired, requireRole("admin"), async (req, res) => {
  const { jobId } = req.query;
  const q = {};
  ...
  const out = docs.map(m => ({
    ...
    location: m.location,
    applicant: (() => {
      const u = uMap.get(m.user.toString());
...
app.patch("/api/v2/meetings/:id", authRequired, requireRole("admin"), async (req, res) => {
  const patch = {};
  ["status", "startsAt", "endsAt", "mode", "location"].forEach(k => {
    if (req.body?.[k] !== undefined) patch[k] = ["startsAt", "endsAt"].includes(k) ? new Date(req.body[k]) : req.body[k];
  });
  patch.updatedAt = new Date();
...
app.delete("/api/v2/meetings/:id", authRequired, requireRole("admin"), async (req, res) => {
  await Meetings.deleteOne({ _id: new ObjectId(req.params.id) });
  res.json({ ok: true });
});

// LinkedIn Dummy
app.post("/api/v2/linkedin/publish/:jobId", authRequired, requireRole("admin"), async (req, res) => {
...
app.delete("/api/v2/linkedin/publish/:jobId", authRequired, requireRole("admin"), async (req, res) => {
  await Jobs.updateOne(
    { _id: new ObjectId(req.params.jobId) },
    { $unset: { linkedinPostId: "" }, $set: { updatedAt: new Date() } }
  );
  res.json({ ok: true });
});

app.post("/api/v2/linkedin/import-applicant", authRequired, requireRole("admin"), async (req, res) => {
  try {
      ...
      if (job) {
        try {
          const doc = {
            ...
            updatedAt: new Date(),
            source: "linkedin",
            sourceProfile: profileUrl || null,
          };
          const r = await Applications.insertOne(doc);
          applicationId = r.insertedId.toString();
        } catch (e) {
          if (e?.code === 11000) {
            const existing = await Applications.findOne({ job: job._id, user: user._id });
    ...
    console.error("linkedin:import-applicant", e);
...
function startGdprWorker() {
  ...
  setInterval(async () => {
    try {
      const now = new Date();
      await Applications.deleteMany({ deleteAt: { $lte: now } }); // TTL-Index räumt i.d.R. ebenfalls auf
    } catch (e) {
...
// ---------- Process-Level Error Logging (nicht crashen) ----------
