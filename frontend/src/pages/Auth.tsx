...
export default function Auth({ onLogin }:{ onLogin: (token:string, role:string)=>void }) {
  ...
  async function login() {
    try {
      ...
      // Rolle aus Token-Antwort (res.role) oder /auth/me
  ...
  async function register() {
    try {
      await API.post<any>(apiUrl("/public/register"), {
        email: rEmail, password: rPw, birthdate: rBirth || undefined, address: rAddr || undefined
      });
    ...
    <Container>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="card">
          ...
          <p className="mt-3 text-xs text-slate-500">Admin-Login gemäss deinen Seed-Daten: admin@chocadies.local / secret123</p>
        </div>
        ...
        <div className="card">
          ...
          <div className="grid gap-2">
            <div>
              <div className="label">E-Mail</div>
              <input className="input" value={rEmail} onChange={e=>setREmail(e.target.value)} />
            </div>
            <div>
              <div className="label">Passwort</div>
              <input className="input" type="password" value={rPw} onChange={e=>setRPw(e.target.value)} />
            </div>
            <div>
              <div className="label">Geburtsdatum</div>
              <input className="input" type="date" value={rBirth} onChange={e=>setRBirth(e.target.value)} />
            </div>
            <div>
              <div className="label">Adresse</div>
              <input className="input" value={rAddr} onChange={e=>setRAddr(e.target.value)} />
            </div>
