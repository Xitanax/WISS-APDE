...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/health").then(r => r.json()).then(setOk).catch(() => setOk("error"));
  }, []);
  return (
    <Container>
      <div className="grid gap-6 md:grid-cols-2">
        <div className="card">
          ...
          <p className="text-slate-600">Recruiting-Plattform (SPA) mit Meetings, LinkedIn-Dummy & Partner-API.</p>
          <div className="mt-4 text-sm text-slate-500">Backend Health: <b>{ok || "…"}</b></div>
        </div>
        <div className="card">
          <h2 className="font-semibold mb-2">Schnellzugriff</h2>
